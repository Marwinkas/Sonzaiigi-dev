...
export default function Register() {
    ...
    useEffect(() => { document.title = "Регистрация | Sonzaiigi"; }, []);
        ...
        <Box sx={{ minHeight: '100vh', bgcolor: '#0b1120', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
            <Paper elevation={0} sx={{ 
                ...
                <Box component="form" onSubmit={submit}>
                    ...
                    {/* Поле Имя */}
                    <TextField
                        fullWidth label="Отображаемое имя" required
                    ...
                    {/* Поле Username */}
                    <TextField
                        fullWidth label="Юзернейм (@username)" required
                        value={data.username} onChange={e => setData({ ...data, username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') })}
                        sx={inputStyle}
                        helperText="Только латинские буквы и цифры"
                        FormHelperTextProps={{ sx: { color: '#64748b', ml: 0 } }}
                        InputProps={{
